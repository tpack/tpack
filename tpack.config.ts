		...
		compilers: [
			...
			},
			{
				...
				use: [{
					...
					options: { path: "./tsconfig.json", noTypeCheck: true, declaration: true, target: "es2018", module: "commonjs", sourceMap: false }
				}]
			},
		],
		bundler: {
...
export async function updateMimeTypes() {
	const mimeTypes = {}
	const db = (await request("https://raw.githubusercontent.com/jshttp/mime-db/master/db.json")).json as { [mimeType: string]: { extensions?: string[] } }
