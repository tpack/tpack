...
export function getFileIcon(fileName: string) {
	...
	if (icon) {
		return readIcon(fileIcons, "file", icon)
	}
...
export function getDirIcon(dirName: string) {
	...
	if (icon) {
		return readIcon(dirIcons, "dir", icon)
	}
...
export function getDefaultIcon(type: "back" | "dir" | "file" | "home" | "root") {
	return readIcon(defaultIcons, "default", type)
}

/** 读取图标数据 */
function readIcon(cache: Map<string, string>, dir: string, name: string) {
