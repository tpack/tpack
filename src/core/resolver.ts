...
import { relativePath, setName } from "../utils/path"
...
export class Resolver {
	...
	constructor(options: ResolverOptions = {}, fs = new FileSystem()) {
		...
		this.extensions = options.extensions || (browser ? ["", ".wasm", ".tsx", ".ts", ".jsx", ".mjs", ".js", ".json"] : ["", ".js", ".json", ".node"])
	}
	...
	protected async resolveNoCache(moduleName: string, containingDir: string, trace?: string[], ignoreAliasField?: boolean, ignoreAlias?: boolean): Promise<string | null | false> {
			...
			for (const alias of this.alias) {
				...
				for (const replacement of alias.replacements) {
					if (replacement === false) {
						trace && trace.push(i18n`Apply alias ${alias.match} -> Got false`)
						return false
					}
					const replaced = moduleName.replace(alias.match, replacement as any)
					trace && trace.push(i18n`Apply alias ${alias.match} -> '${replaced}'`)
		...
		if (!ignoreAliasField && this.aliasFields.length) {
			...
			if (descriptionFile) {
				if (descriptionFile.aliasField === undefined) {
					trace && trace.push(i18n`Test field '${this.aliasFields.join("/")}' in '${descriptionFile.path}' -> Not found`)
				} else if (typeof descriptionFile.alias !== "object" || descriptionFile.alias === null) {
					trace && trace.push(i18n`Test field '${descriptionFile.aliasField}' in '${descriptionFile.path}' -> Skipped, not an object`)
				} else {
					const aliasName = descriptionFile.alias[moduleName]
					if (typeof aliasName === "string") {
						trace && trace.push(i18n`Apply alias field '${descriptionFile.aliasField}["${moduleName}"]' in '${descriptionFile.path}' -> '${aliasName}'`)
						...
						trace && trace.push(i18n`Test field '${descriptionFile.aliasField}["${moduleName}"]' in '${descriptionFile.path}' -> Got false`)
						return false
					} else {
						trace && trace.push(aliasName === undefined ? i18n`Test field '${descriptionFile.aliasField}["${moduleName}"]' in '${descriptionFile.path}' -> Not found` : i18n`Test field '${descriptionFile.aliasField}["${moduleName}"]' in '${descriptionFile.path}' -> Skipped, not a string or false`)
					}
				}
			}
		}
		...
		for (const moduleDirectory of this.modules) {
			if (moduleDirectory.absolute) {
				const result = await this.resolveFileOrDir(moduleName, moduleDirectory.name, trace, ignoreAliasField)
				if (result !== null) {
					return result
				}
			} else {
				...
				while (true) {
					// 跳过 node_modules 本身
					if (basename(currentDir) !== moduleDirectory.name) {
						const result = await this.resolveFileOrDir(moduleName, join(currentDir, moduleDirectory.name), trace, ignoreAliasField)
						if (result !== null) {
							return result
						}
					}
	...
	protected async resolveFileOrDir(moduleName: string, containingDir: string, trace?: string[], ignoreAliasField?: boolean, ignoreMainFields?: boolean, noExtension?: boolean): Promise<string | null | false> {
		...
		try {
			...
			trace && trace.push(i18n`Test '${dir}' -> ${e.message}`)
			return null
		}
		// 文件夹不存在(false)或是一个文件(null)
		if (!entries) {
			trace && trace.push(entries === null ? i18n`Test '${dir}' -> Not found` : i18n`Test '${dir}' -> Skipped, not a directory`)
			return null
		}
		// 路径存在但大小写错误
		...
		if (entries.realPath) {
			...
			if (realName !== actualName) {
				trace && trace.push(i18n`Case mismatched: '${moduleName}' should be '${(prefix || (moduleName.startsWith("./") ? "./" : "")) + (sep === "\\" ? realName.replace(/\\/g, "/") : realName)}'`)
				return null
			}
		}
		// 测试文件
			...
			for (const extension of noExtension && this.extensions[0] !== "" ? ["", ...this.extensions.filter(extension => extension)] : this.extensions) {
				...
				if (!entries.has(this.ignoreCase ? (name + extension).toLowerCase() : name + extension)) {
					if (trace) {
						...
						if (matched) {
							trace.push(i18n`Case mismatched: '${moduleName}' should be '${setName(moduleName, matched.endsWith(extension) ? matched.substring(0, matched.length - extension.length) : matched)}'`)
						} else {
							trace.push(i18n`Test '${path + extension}' -> Not found`)
						}
					}
					continue
				}
				...
				try {
					...
					trace && trace.push(i18n`Test '${fullPath}' -> ${e.message}`)
					continue
				}
				if (existsFile) {
					trace && trace.push(i18n`Test '${fullPath}' -> Succeed`)
					// 应用所在包的别名
					if (this.aliasFields.length && !ignoreAliasField) {
						...
						if (descriptionFile) {
							if (descriptionFile.aliasField === undefined) {
								...
								if (typeof aliasName === "string") {
									trace && trace.push(i18n`Apply alias field '${descriptionFile.aliasField}["${name}"]' in '${descriptionFile.path}' -> '${aliasName}'`)
									const result = await this.resolveNoCache(aliasName, dirname(descriptionFile.path), trace, true, undefined)
									if (result !== null) {
										return result
									}
								} else if (aliasName === false) {
									trace && trace.push(i18n`Test field '${descriptionFile.aliasField}["${name}"]' in '${descriptionFile.path}' -> Got false`)
									return false
								} else {
									trace && trace.push(aliasName === undefined ? i18n`Test field '${descriptionFile.aliasField}["${name}"]' in '${descriptionFile.path}' -> Not found` : i18n`Test field '${descriptionFile.aliasField}["${name}"]' in '${descriptionFile.path}' -> Skipped, not a string or false`)
								}
							}
						}
					}
				...
				trace && trace.push(existsFile === null ? i18n`Test '${fullPath}' -> Not found` : i18n`Test '${fullPath}' -> Skipped, not a file`)
			}
		}
		// 测试文件夹
		if (noExtension !== false) {
			if (entries.has(this.ignoreCase ? name.toLowerCase() : name)) {
				...
				if (!ignoreMainFields && this.mainFields.length) {
					...
					if (descriptionFile) {
						if (descriptionFile.mainField === undefined) {
							trace && trace.push(i18n`Test field '${this.mainFields.join("/")}' in '${descriptionFile.path}' -> Not found`)
						} else {
							const mainValue = descriptionFile.main
							if (typeof mainValue !== "string") {
								trace && trace.push(i18n`Test field '${descriptionFile.mainField}' in '${descriptionFile.path}' -> Skipped, not a string`)
							} else if (!mainValue) {
								trace && trace.push(i18n`Test field '${descriptionFile.mainField}' in '${descriptionFile.path}' -> Skipped, is empty string`)
							} else {
								trace && trace.push(i18n`Test field '${descriptionFile.mainField}' in '${descriptionFile.path}' -> '${join(path, mainValue)}'`)
				...
				if (matched) {
					trace.push(i18n`Case mismatched: '${moduleName}' should be '${setName(moduleName, matched)}'`)
				} else {
					trace.push(i18n`Test '${path}' -> Not found`)
				}
			}
		}
		return null
	...
	/** 描述文件数据的缓存 */
	private readonly _descriptionFileCache: Map<string, DescriptionFile | null | Promise<DescriptionFile | null>>
	...
	protected async readDescriptionFileNoCache(dir: string, trace?: string[]) {
		for (const descriptionFileName of this.descriptionFiles) {
			...
			try {
				...
				if (e.code === "ENOENT") {
					trace && trace.push(i18n`Test '${descriptionFilePath}' -> Not found`)
					continue
				}
				trace && trace.push(i18n`Test '${descriptionFilePath}' -> ${e.message}`)
				return { path: descriptionFilePath }
			}
			// 解析 JSON 文件
			const result: DescriptionFile = {
			...
			try {
				...
				trace && trace.push(i18n`Test '${descriptionFilePath}' -> Cannot parse JSON: ${e.message}`)
				return result
			}
			if (!descriptionFileData || typeof descriptionFileData !== "object") {
				trace && trace.push(i18n`Test '${descriptionFilePath}' -> Skipped, not a object`)
				return result
			}
			// 读取字段
			...
			trace && trace.push(i18n`Test '${descriptionFilePath}' -> Ok`)
			return result
		}
		return null
	}
