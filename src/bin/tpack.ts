...
async function main() {
	...
	const { i18n, localeService, translate } = require("../core/i18n") as typeof import("../core/i18n")
	if (/^v\d\.|^v10\.\d\.|^v10\.1[01]\./.test(process.version)) {
		console.error(i18n`TPack requires Node.js >= 10.12, currently ${process.version}`)
	...
	// 定义命令行参数
	const { parseCommandLineArguments, formatCommandLineOptions } = require("../utils/commandLine") as typeof import("../utils/commandLine")
	const commandLineOptions = {
		"--help": {
			...
			execute() {
				...
				if (!localeService.isDefaultLocale) {
					for (const key in commandLineOptions) {
						const commandLineOption = commandLineOptions[key as keyof typeof commandLineOptions] as typeof commandLineOptions["--help"]
						if (commandLineOption.group) {
							commandLineOption.group = translate(commandLineOption.group)
						}
						if (commandLineOption.description) {
							commandLineOption.description = translate(commandLineOption.description)
						}
					}
				}
	...
	if (commandLineErrors) {
		for (const commandLineError of commandLineErrors) {
			console.error(i18n`CommandLineError: ${translate(commandLineError)}`)
		}
		process.exitCode = -1
		return
	}
	...
	// 解析配置文件
	const { extensions, loadConfig } = require("../core/config") as typeof import("../core/config")
	...
	if (taskNames.length !== 1) {
		console.error(i18n`Task '${taskName || "default"}' is not defined in '${configFile || i18n`<default config file>`}'`)
		console.info("")
		if (taskNames.length) {
			...
			console.info(i18n`Use ES6 export syntax to define a task. e.g. export default function (){ }`)
			process.exitCode = -5
		}
		return
	}
	...
	if (configFile && args["--cwd"] === undefined) {
