...
export class MemoryFileSystem extends FileSystem {
	 ...
	 */
	async getStat(path: string, resolveLink = true) {
	 ...
	 * @param path 文件夹内的文件路径，如果路径是相对路径则当前文件夹及上级文件夹不会被删除
	...
	async deleteParentDirIfEmpty(path: string): Promise<number> {
		...
		return (await this.deleteParentDirIfEmpty(parent)) + 1
	}
	...
	async walk(path: string, options: WalkOptions) {
		...
		switch (data) {
			case null:
				...
				break
			case undefined:
				options.error && options.error(this.createError("scandir", "ENOENT", -4058, path, `ENOENT: no such file or directory, scandir '${path}'`), path)
				break
			default:
	...
	async appendFile(path: string, data: string | Buffer) {
		...
		await this.writeFile(path, Buffer.concat([original, typeof data === "string" ? Buffer.from(data) : data]))
	}
	 ...
	 * @returns 如果复制成功则返回 `true`，否则说明目标已存在，返回 `false`
	 */
	async copyLink(src: string, dest: string, overwrite = true): Promise<boolean> {
		throw this.createError("link", "EPERM", -4048, dest, `EPERM: operation not permitted, link '${src}' -> '${dest}'`)
	}
	 ...
	 * @returns 如果移动成功则返回 `true`，否则说明目标已存在，返回 `false`
	 */
	async moveLink(src: string, dest: string, overwrite = true): Promise<boolean> {
		throw this.createError("link", "EPERM", -4048, dest, `EPERM: operation not permitted, link '${src}' -> '${dest}'`)
	}
