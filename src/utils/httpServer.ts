...
export class HTTPServer {
	...
	set sessions(value) {
		this._sessions && this._sessions.close()
...
export class HTTPRequest extends IncomingMessage {
	...
	/** 获取客户端的证书信息 */
	get certificate() { return (this.socket as TLSSocket).encrypted ? (this.socket as TLSSocket).getPeerCertificate(true) : undefined }
	...
	/** 获取当前请求的文本内容，如果当前请求不包含内容则返回空 */
	get text() {
		if (this._text === undefined && this.body !== undefined) {
			this._text = this.body.toString(this.contentEncoding)
		}
		return this._text
	}
	...
	/** 获取当前请求的 JSON 数据，如果当前请求不包含 JSON 数据则返回空 */
	get json() {
...
function parseMultipart(body: Buffer, boundary: string) {
	...
	outer: while (index < body.length) {
		...
		while (true) {
			...
			if (body[index] === 13 /*\r*/ && body[index + 1] === 10 /*\n*/) {
				index += 2
				addField(result, name, fileName !== undefined ? new HTTPFile(fileName, headers, body, startIndex, endIndex - 2) : body.toString(parseCharset(headers["content-type"] as string), startIndex, endIndex - 2))
				continue outer
			}
...
export class HTTPFile {
	...
	/** 获取请求文件的文本内容 */
	get text() { return this.body.toString(this.contentEncoding) }
...
export class HTTPSessionManager {
	...
	getSession(request: HTTPRequest, response: HTTPResponse) {
		...
		if (!data || data.expires <= Date.now()) {
			response.setCookie(this.cookieName, sessionID = this.createSessionId(request), this.maxAge, this.domain, undefined, true, this.secure)
