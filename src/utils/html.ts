...
export function decodeHTML(value: string) {
	...
	return value.replace(/&(#(\d+)|#x([\da-f]+)|\w+);/ig, (source, entity?: string, unicode?: string, unicodeHex?: string) => {
		if (unicode) {
			return String.fromCodePoint(parseInt(unicode))
		}
		if (unicodeHex) {
			return String.fromCodePoint(parseInt(unicodeHex, 16))
		}
