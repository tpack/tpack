import { IncomingMessage, OutgoingHttpHeaders, request as nativeHTTPRequest, STATUS_CODES } from "http"
import { request as nativeHTTPSRequest, RequestOptions } from "https"
import { stringify } from "querystring"
import { Readable } from "stream"
import { parse, UrlObject, UrlWithStringQuery, resolve } from "url"
import { createBrotliDecompress, createGunzip, createInflate } from "zlib"

/**
 * 创建并发送一个新的 HTTP/HTTPS 请求
 * @param url 要请求的地址或附加选项
 * @param callback 服务器响应后的回调函数
 * @param callback.error 错误对象
 * @param callback.response 服务器的响应对象
 * @param callback.url 当前实际请求的地址对象
 * @returns 返回本次请求对象，可以调用 `.abort()` 终止请求
 */
export function createHTTPRequest(url: string | URL | HTTPClientRequestOptions, callback = (error: Error | null, response: HTTPClientResponse, url?: UrlObject | URL) => { }) {
	const options: HTTPClientRequestOptions & RequestOptions = typeof url === "string" || url instanceof URL ? { url: url } : { ...url }
	// 分析请求地址
	const urlObject = typeof options.url === "string" ? parse(options.url) : options.url
	options.protocol = urlObject.protocol
	options.hostname = urlObject.hostname
	options.port = urlObject.port
	options.auth = (urlObject as UrlWithStringQuery).auth || ((urlObject as URL).username != undefined && (urlObject as URL).password != undefined ? `${(urlObject as URL).username}:${(urlObject as URL).password}` : (urlObject as URL).username)
	options.path = `${urlObject.pathname || ""}${urlObject.search || ""}` || "/"
	const headers: OutgoingHttpHeaders = {
		"accept-encoding": "gzip, deflate"
	}
	if (options.userAgent !== null) {
		headers["user-agent"] = options.userAgent || "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/70.0.0.0 Safari/537.36"
	}
	if (options.cookieJar) {
		headers["cookie"] = options.cookieJar.getCookiesHeader(options.url)
	}
	let data = options.data
	if (data != undefined) {
		if (options.method === "GET") {
			options.path += `${options.path.includes("?") ? "&" : "?"}${stringify(data)}`
			data = undefined
		} else {
			if (!options.method) {
				options.method = "POST"
			...
			if (options.dataType) {
				...
				// 如果 data 是流，或其它无法获取长度的类型，则不设置长度
	...
	const request = (!options.protocol || options.protocol === "http:" ? nativeHTTPRequest : nativeHTTPSRequest)(options, (response: HTTPClientResponse) => {
		...
		if (options.cookieJar) {
			...
			if (setCookies) {
				...
				for (const setCookie of setCookies) {
					options.cookieJar.setCookiesFromHeader(options.url, setCookie, now)
				}
			}
		}
		// 支持跳转
		...
		if (statusCode >= 300 && statusCode < 400 && (options.maxRedirects === undefined || options.maxRedirects > 0)) {
			const location = response.headers["location"]
			if (location !== undefined) {
				response.resume()
				options.url = resolve(options.url.toString(), location)
				// 301, 302：将 POST 改为 GET，重发请求
				if ((statusCode === 301 || statusCode === 302) && options.method && options.method !== "GET" && options.method !== "HEAD") {
					options.method = "GET"
					options.data = undefined
					delete headers["content-length"]
					delete headers["content-type"]
				}
				options.maxRedirects = options.maxRedirects === undefined ? 9 : options.maxRedirects - 1
				const newRequest = createHTTPRequest(options, callback)
				request.on("abort", () => {
					newRequest.abort()
				})
				return
		...
		stream.on("end", () => {
			...
			callback(null, response, urlObject)
		})
	})
	request.on("error", callback)
	let timer: ReturnType<typeof setTimeout> | undefined
	if (options.timeout > 0 && Number.isFinite(options.timeout)) {
		timer = setTimeout(() => {
			request.abort()
		}, options.timeout)
	}
	if (data != undefined && typeof data === "object" && typeof data.pipe === "function") {
		data.pipe(request)
	} else {
		request.end(data)
	}
...
export interface HTTPClientRequestOptions extends Pick<RequestOptions, Exclude<keyof RequestOptions, "protocol" | "hostname" | "host" | "port" | "auth" | "path">> {
	 ...
	 */
	dataType?: "json" | "form" | "multipart" | "plain" | string
 ...
 * @param domain 要获取的域名
 */
function getCookieDomain(domain?: string) {
 ...
 */
function getCookiePath(pathname?: string) {
 ...
 * @param url 要请求的地址或附加选项
 */
export function request(url: string | URL | HTTPClientRequestOptions) {
	return new Promise<any>((resolve, reject) => {
		createHTTPRequest(url, (error, response) => {
			if (error) {
				reject(error)
				...
				const error = new Error(`${response.statusCode} - ${STATUS_CODES[response.statusCode!]}`) as any
