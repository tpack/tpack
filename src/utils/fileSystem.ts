...
export class FileSystem {
	...
	walk(path: string, options: WalkOptions) {
		return new Promise<void>((resolve, reject) => {
			...
			function walk(path: string, stats?: Dirent | Stats) {
				pending++
				if (!stats || stats.isDirectory()) {
					safeCall(readdir, [path || ".", { withFileTypes: true }], (error: NodeJS.ErrnoException | null, entries: Dirent[]) => {
						try {
							if (error) {
								if (error.code === "ENOTDIR") {
									(options.follow !== false ? stat : lstat)(path, (error, stats) => {
										try {
											if (error) {
							...
							} else if (!options.dir || options.dir(path, entries) !== false) {
								for (const entry of entries) {
	 ...
	 * @param baseDir 如果要匹配绝对路径，则设置匹配的根绝对路径
	 * @returns 返回所有匹配文件的路径
	 */
	...
	async readText(path: string, throwIfNotFound?: boolean) {
		try {
			...
			if (throwIfNotFound === false && e.code === "ENOENT") {
				return null
			}
			throw e
		}
...
export interface WalkOptions {
	 ...
	 */
	dir?(path: string, entries: Dirent[]): boolean | void
	/**
	 * 处理一个文件的回调函数
	 * @param path 当前文件的路径
	 */
	 ...
	 * @param stats 文件的属性
	 */
	other?(path: string, stats: Dirent | Stats): void
}
