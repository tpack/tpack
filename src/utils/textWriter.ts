...
export class TextWriter {
	 ...
	 */
	write(content: string, startIndex = 0, endIndex = content.length, sourcePath?: string | number, sourceLine?: number, sourceColumn?: number, name?: string | number, sourceMap?: SourceMapBuilder) {
...
export class SourceMapTextWriter extends TextWriter {
	...
	write(content: string, startIndex = 0, endIndex = content.length, sourcePath?: string | number, sourceLine?: number, sourceColumn?: number, name?: string | number, sourceMap?: SourceMapBuilder) {
		...
		if (sourceColumn === undefined) {
			...
			let mappings = sourceMap.mappings[sourceLine!] || []
			let mappingIndex = 0
			...
			if (!foundStartMapping && startIndex < endIndex) {
				if (mappingIndex > 0) {
					...
					if (prevMapping.sourceIndex === undefined) {
						...
						sourceMapBuilder.addMapping(line, column, sourceMappings[prevMapping.sourceIndex] = sourceMapBuilder.addSource(sourceMap.sources[prevMapping.sourceIndex]), prevMapping.sourceLine!, prevMapping.sourceColumn! + (sourceColumn! - prevMapping.generatedColumn), prevMapping.nameIndex === undefined ? undefined : sourceMap.names[prevMapping.nameIndex])
					}
				} else {
					sourceMapBuilder.addMapping(line, column)
				}
			...
			for (; startIndex < endIndex; startIndex++) {
				switch (content.charCodeAt(startIndex)) {
					...
					case 10  /*\n*/:
						...
						mappings = sourceMap.mappings[++sourceLine!] || []
						break
					default:
						if (isLineStart) {
							if (startIndex > lastIndex) {
							...
							if (mapping.generatedColumn === sourceColumn) {
								...
								if (mapping.sourceIndex === undefined) {
									...
									if (newSourceIndex === undefined) {
										sourceMappings[mapping.sourceIndex] = newSourceIndex = sourceMapBuilder.addSource(sourceMap.sources[mapping.sourceIndex])
									}
									sourceMapBuilder.addMapping(line, column, newSourceIndex, mapping.sourceLine!, mapping.sourceColumn!, mapping.nameIndex === undefined ? undefined : sourceMap.names[mapping.nameIndex])
								}
							}
						}
						column++
