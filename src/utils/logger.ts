...
export class Logger {
	...
	formatLog(log: string | Error | LogEntry, level = LogLevel.log, colors = this.colors) {
		...
		switch (level) {
			...
			case LogLevel.warning:
				...
				break
			case LogLevel.success:
				result = `${this.timestamp ? color(formatDate(new Date(), "[HH:mm:ss] "), ANSIColor.brightBlack) : ""}${color(this.successIcon, ANSIColor.brightGreen)}${result}`
				break
			case LogLevel.fatal:
				result = `${this.timestamp ? color(formatDate(new Date(), "[HH:mm:ss] "), ANSIColor.brightBlack) : ""}${color(this.fatalIcon, ANSIColor.brightRed)}${result}`
				break
			case LogLevel.info:
			case LogLevel.debug:
				result = `${this.timestamp ? color(formatDate(new Date(), "[HH:mm:ss] "), ANSIColor.brightBlack) : ""}${result}`
				break
		}
		// 去除颜色信息
		if (!colors) {
	...
	begin(taskName: string, detail?: string, persistent?: boolean) {
		...
		if (this.logLevel === LogLevel.debug) {
			this.debug(`${color(`+`, ANSIColor.brightMagenta)} ${content}`)
		} else {
	 ...
	 */
		...
		if (this.logLevel === LogLevel.debug) {
			this.debug(`${color(`-`, ANSIColor.brightBlue)} ${taskId.content}`)
