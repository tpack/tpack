...
export function deleteParentDirIfEmpty(path: string): number {
	...
	return deleteParentDirIfEmpty(parent) + 1
}
...
export function walk(path: string, options: WalkOptions, /**@internal */ _stats?: Dirent | Stats) {
	if (!_stats || _stats.isDirectory()) {
		...
		options.other && options.other(path, _stats)
	}
}
	...
	try {
		if (process.platform === "win32") {
			...
			try {
				...
				if (e.code === "EPERM") {
					if (isDir) {
						symlinkSync(resolvePath(path, "..", target), path, "junction")
					} else {
 ...
 * @returns 如果复制成功则返回 `true`，否则说明目标已存在，返回 `false`
 */
export function copyLink(src: string, dest: string, overwrite = true): boolean {
	try {
		createLink(dest, readLink(src), overwrite)
	} catch (e) {
		try {
			linkSync(src, dest)
		} catch (e) {
			if (e.code === "EEXIST") {
				if (overwrite) {
					deleteFile(dest)
					return copyLink(src, dest, false)
				} else {
					return false
				}
			} else {
