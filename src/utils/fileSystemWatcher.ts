...
export class FileSystemWatcher extends EventEmitter {
	...
	private _createWatcher(path: string, callback?: (error: NodeJS.ErrnoException | null, success: boolean) => void, initStats?: boolean) {
		...
		const initWatcher = (error: NodeJS.ErrnoException | null) => {
			const stats = this._stats.get(path)
			...
			}).on("change", stats !== undefined && typeof stats[0] === "number" ? () => {
	...
	/**
	 * 所有文件或文件夹状态，对象的键是路径
	 * - 如果路径是一个文件，则值为文件的最后修改时间戳和最后大小组成的数组
	 * - 如果路径是一个文件夹，则值为所有直接子文件和子文件夹的名称数组
	 */
	private readonly _stats = new Map<string, string[] | [number, number]>()
		...
		if (isFile) {
			stat(path, (error, stats) => {
				if (error) {
					...
					this._stats.set(path, [stats.mtimeMs, stats.size])
	...
	private _resolveUpdates = () => {
		...
		if (pendingUpdates.size > 1) {
			...
			for (const pendingChange of pendingUpdates) {
				...
				this._commitUpdate(pendingChange, stats !== undefined && typeof stats[0] === "number", pendingUpdates)
			}
			...
			for (const pendingChange of pendingUpdates) {
				const stats = this._stats.get(pendingChange)
				this._commitUpdate(pendingChange, stats !== undefined && typeof stats[0] === "number")
			}
	 ...
	 */
	private _commitUpdate(path: string, isFile: boolean, pendingUpdates?: Set<string>) {
		this._pending++
		if (isFile) {
			stat(path, (error, stats) => {
				if (error) {
					...
					if (prevStats !== undefined && typeof prevStats[0] === "number") {
						const prevWriteTime = prevStats[0]
						const prevSize = prevStats[1] as number
						// 文件 -> 文件
						if (prevWriteTime !== newWriteTime || prevSize !== newSize) {
							prevStats[0] = newWriteTime
							prevStats[1] = newSize
							this.onChange(path, stats, prevWriteTime, prevSize)
						}
					} else {
						// * -> 文件
						...
						this._stats.set(path, [newWriteTime, newSize])
						this.onCreate(path, stats)
					}
				} else if (stats.isDirectory()) {
					// * -> 文件夹
					this._commitUpdate(path, false, pendingUpdates)
				}
				if (--this._pending === 0) {
					this.emit("ready")
				}
			...
			readdir(path, (error, entries) => {
				if (error) {
					if (error.code === "ENOENT") {
						...
						// * -> 文件
						this._commitUpdate(path, true, pendingUpdates)
					} else if (error.code === "EMFILE" || error.code === "ENFILE") {
						this._pending++
						setTimeout(() => {
							this._commitUpdate(path, isFile, pendingUpdates)
							if (--this._pending === 0) {
								this.emit("ready")
							}
						}, this.delay)
					...
					if (prevStats !== undefined && typeof prevStats[0] !== "number") {
						...
						if (this.usePolling) {
							for (const entry of entries as string[]) {
								...
								this._commitUpdate(child, entry.includes(".", 1), pendingUpdates)
							}
						}
						...
						for (const entry of entries) {
							...
							this._commitUpdate(child, childStats !== undefined ? typeof childStats[0] === "number" : entry.includes(".", 1), pendingUpdates)
						}
					}
				}
				if (--this._pending === 0) {
	...
	private _commitDelete(path: string, pendingUpdates?: Set<string>) {
		...
		if (typeof prevStats[0] === "number") {
			this.onDelete(path, prevStats[0], prevStats[1] as number)
		} else {
			for (const entry of prevStats as string[]) {
				const child = join(path, entry)
	 ...
	 * @param prevSize 文件被删除前最后一次的大小
	 */
	protected onChange(path: string, stats: Stats, prevWriteTime: number, prevSize: number) { this.emit("change", path, stats, prevWriteTime, prevSize) }
	 ...
	 * @param prevSize 文件被删除前最后一次的大小
	 */
	protected onDelete(path: string, prevWriteTime: number, prevSize: number) { this.emit("delete", path, prevWriteTime, prevSize) }
