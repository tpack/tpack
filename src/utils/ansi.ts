 ...
 * @param items[].content 节点内容
 * @param maxWidth 允许显示的最大宽度（一般地，西文字母宽度为 1，中文文字宽度为 2）
 */
export function formatTree(items: readonly { indent: number, icon?: string, content: string }[], maxWidth = process.stdout.columns || Infinity) {
	let result = ""
	const lasts: number[] = []
	for (let i = 0; i < items.length; i++) {
		...
		const rows = wrapString(item.content, lastsEnd >= 0 ? 0 : 2, maxWidth - getStringWidth(prefix))
...
/**
 * 格式化一个代码片段
 * @param content 要格式化的代码片段（不支持 ANSI 控制字符）
