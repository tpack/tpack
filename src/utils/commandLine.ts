				...
				if (commandLineOption.argument) {
					if (value === undefined) {
						if (startIndex + 1 < argv.length && argv[startIndex + 1].charCodeAt(0) !== 45 /*-*/) {
							...
							onError && onError(`Option '${argument}' requires an argument`)
							continue
						}
					}
					if (commandLineOption.multiple) {
						...
						if (oldValue !== undefined) {
							onError && onError(`Duplicate option '${argument}'`)
						}
						result[key] = value!
					}
					...
					if (value !== undefined) {
						onError && onError(`Option '${argument}' has no argument, got '${value}'`)
					}
					result[key] = true
				}
			} else {
